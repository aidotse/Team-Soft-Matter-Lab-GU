...
class Generator(keras.utils.Sequence):
    ...
    def generate(
        ...
        while True:
            with threading.Lock():
                ...
                contains_class = [
                    [] for _ in range(len(ensure_contains_classes))
                ]
                while (not all(contains_class)) or len(batch) < batch_size:
                    ...
                    for list_index, class_list in enumerate(contains_class):
                        if np.any(
                            labels[-1] == ensure_contains_classes[list_index]
                        ):
    ...
    def _get(
        self, features: Feature or List[Feature], feature_kwargs
    ) -> Image:
        ...
        if isinstance(features, List):
            for feature in features:
                feature.update()
            return [
                feature.resolve(**feature_kwargs)
